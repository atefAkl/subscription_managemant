...
@section('content')
<x-breadcrumb :items="[
    [ 'label' => 'لوحة الادارة', 'url' => route('admin.dashboard') ], 
    [ 'label'=> 'طلبات الاشتراك', 'url' => '' ],
    ]" />
<!-- Main Content -->
                        ...
                        <div class="w-8 h-8 bg-blue-500 rounded-md flex items-center justify-center">
                            <svg class="w-5 h-5 text-white" fill="currentColor" viewBox="0 0 20 20">
                                ...
                                <path fill-rule="evenodd"
                                    d="M18 9H2v5a2 2 0 002 2h12a2 2 0 002-2V9zM4 13a1 1 0 011-1h1a1 1 0 110 2H5a1 1 0 01-1-1zm5-1a1 1 0 100 2h1a1 1 0 100-2H9z" clip-rule="evenodd">
                                </path>
                            </svg>
                        </div>
                    </div>
    ...
    <div class="bg-white shadow rounded-lg mb-6">
        <div class="border-b border-gray-200">
            <nav class="-mb-px flex space-x-8 space-x-reverse px-6" aria-label="Tabs">
                <a href="{{ route('admin.subscription-requests.index') }}" class="border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm
                       {{ request('status', 'all') == 'all' ? 'border-blue-500 text-blue-600' : '' }}">
                    الكل ({{ $totalRequests }})
                </a>
                <a href="{{ route('admin.subscription-requests.index', ['status' => 'pending']) }}" class="border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm
                       {{ request('status') == 'pending' ? 'border-blue-500 text-blue-600' : '' }}">
                    معلقة ({{ $stats['pending'] }})
                </a>
                <a href="{{ route('admin.subscription-requests.index', ['status' => 'quoted']) }}" class="border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm
                       {{ request('status') == 'quoted' ? 'border-blue-500 text-blue-600' : '' }}">
                    تم تسعيرها ({{ $stats['quoted'] }})
                </a>
                <a href="{{ route('admin.subscription-requests.index', ['status' => 'paid']) }}" class="border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm
                       {{ request('status') == 'paid' ? 'border-blue-500 text-blue-600' : '' }}">
                    مدفوعة ({{ $stats['paid'] }})
                </a>
                <a href="{{ route('admin.subscription-requests.index', ['status' => 'active']) }}" class="border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm
                       {{ request('status') == 'active' ? 'border-blue-500 text-blue-600' : '' }}">
                    نشطة ({{ $stats['active'] }})
                </a>
                <a href="{{ route('admin.subscription-requests.index', ['status' => 'rejected']) }}" class="border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300 whitespace-nowrap py-4 px-1 border-b-2 font-medium text-sm
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex space-x-2 space-x-reverse">
                                    ...
                                    @if($request->status == 'pending' && $request->is_demo)
                                    ...
                                    <form method="POST" action="{{ route('admin.payments.verify', $request->payments->first()->id ?? 0) }}" class="inline">
                                        @csrf
                                        <button type="submit" class="text-purple-600 hover:text-purple-900 text-xs bg-purple-100 hover:bg-purple-200 px-2 py-1 rounded">
                                            تفعيل
                                        </button>
                                    </form>
                                    @endif
        ...
        <div class="border-t border-gray-200 px-4 py-8 text-center">
            <svg class="mx-auto h-12 w-12 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    d="9 5H7a2 2 0 00-2 2v10a2 2 0 002 2h8a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"></path>
            </svg>
