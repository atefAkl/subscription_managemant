    ...
    <div class="grid grid-cols-1 ">
        @foreach ($groups as $group)
            <div class="bg-white p-4 rounded-lg shadow-md mb-3">
                ...
                <button data-group-id="{{ $group->id }}" class="btn btn-outline-primary btn-sm" data-toggle="modal"
                    data-target="#editGroupModal{{ $group->id }}"><i class="fa fa-edit"></i> تعديل
                </button>
                <button data-group-id="{{ $group->id }}" class="btn btn-outline-danger btn-sm" data-toggle="modal"
                    data-target="#deleteGroupModal{{ $group->id }}"> <i class="fa fa-trash"></i> حذف
                </button>
                <button data-group-id="{{ $group->id }}" data-group-name="{{ $group->name }}"
                    class="add-group-item-button btn btn-outline-success btn-sm" data-toggle="modal"
                    data-target="#addGroupItemModal"><i class="fa fa-plus-circle"></i> إضافة عنصر للمجموعة
                </button>
                    ...
                    @forelse ($group->group_items as $gItem)
                        <form class="pt-1" action="{{ route('admin.settings.groups.items.update', $gItem->id) }}"
                            ...
                            <div class="input-group py-0">
                                ...
                                <input type="text" name="description" class="form-control" value="{{ $gItem->name }}"
                                    required id="description">
                                <button type="submit" class="input-group-text text-primary"><i
                                        class="fa fa-edit"></i></button>
                                <button type="button" class="input-group-text text-danger"><i class="fa fa-trash"></i>
                                </button>
                            </div>
                        </form>
                    @empty
